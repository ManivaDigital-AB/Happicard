...
from .serializers import OrderSerializer, PaymentSerializer, AddressSerializer
        ...
        if form.is_valid():
            ...
            try:
                ...
                if use_default or save:
                    ...
                    charge = stripe.Charge.create(
                        amount=amount, currency="sek", source=token  # cents
                    )
...
class OrderSummary(LoginRequiredMixin, APIView):
    def get(self, *args, **kwargs):
        try:
            order = Order.objects.get(user=self.request.user, ordered=False)
...
def remove_from_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            return redirect("orders:order", slug=slug)
    else:
...
class AddressCreate(generics.RetrieveUpdateDestroyAPIView):
    permission_classes = (IsAuthenticated,)
    serializer_class = AddressSerializer
